...
public class X_SWEA_D4_1233 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int t = 1; t <= 10; t++) {
			N = Integer.parseInt(input.readLine()); // 노드의 개수
			tree = new String[N + 1]; // 인덱스는 1번부터 시작한다.
			for (int n = 1; n <= N; n++) {
				tokens = new StringTokenizer(input.readLine());
				tokens.nextToken();// index - 그냥 버림
				tree[n] = tokens.nextToken();// 트리에 저장될 데이터
			}
			// 입력 종료
			//preOrder(1);
			//inOrder(1);
			//postOrder(1);
			answer = 1;// 기본적으로는 가능하다고 생각하고 출발해보자.
			solveByPostOrder(1);
	...
	static void solveByPostOrder(int i) {
		...
		// 내 노드 사용하기 - 현재 노드 기준으로 할일, 나는 연산자인가?
		...
		if(oper.contains(node)) {
			...
			// 자식은 있어. 그럼 그녀석들은 다 피연산자여야해..
				...
				// 계산 가능하니.. 나를 피연산자로 바꿔주자!!!
				tree[i] = "0";
			}
		}
		// 노드가 피연산자라면.. 완전 이진 트리니까 왼쪽만 봐도 알겠다~~~ 
		else {
			if(i*2<=N) {
				answer = 0;
				return;
